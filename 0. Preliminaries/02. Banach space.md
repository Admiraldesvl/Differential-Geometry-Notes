...
An elementary example is $\mathbb{R}$. It is a vector space with norm $\lVert x \rVert = \sqrt{x^2}$, which is complete. But some spaces are not. For example, $H(\Omega)$, where $\Omega$ is an open subset on the plane, is not, since it's even not normable.
